...
export function DoctorLogin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(user.role === 'doctor' ? '/doctor/dashboard' : user.role === 'admin' ? '/medical-imaging' : '/staff/dashboard', { replace: true });
    } catch (err) {
  ...
  return (
    <AuthLayout title="Doctor Login" subtitle="Doctor Panel">
      <p className="mb-6 text-sm text-slate-500">Sign in to manage appointments, prescriptions, and patient care.</p>
